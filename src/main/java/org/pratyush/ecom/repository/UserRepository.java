...
import org.pratyush.ecom.db.tables.records.TblUsersRecord;
...
import static org.pratyush.ecom.db.tables.TblRefreshToken.TBL_REFRESH_TOKEN;
    ...
    public Mono<Integer> register(UserRegistrationRequest request) {
        ...
        return Mono.from(
                        dsl.insertInto(TBL_USERS)
                                .set(record))
                .flatMap(i -> roleRepository.addDefaultRoleForUser(request)).log()
                .onErrorReturn(0);
    }
